...
#include "chromosome_junctions.h"
#include "Fish.h"
...
Rcpp::List simulate_cpp(Rcpp::NumericVector input_population,
 ...
 // typedef Fish<chromosome_junctions> FOCAL_ANIMAL;
  using FOCAL_ANIMAL = Fish<chromosome_junctions>;
  ...
  if (input_population[0] > -1e4) {
    ...
    Pop = convert_NumericVector_to_fishVector<FOCAL_ANIMAL,
...
std::vector< std::vector< FISH > > simulate_two_populations(
  ...
  if (use_selection) {
    ...
    for (size_t i = 0; i < pop_1.size(); ++i) {
      fitness_pop_1[i] = calc_fitness(pop_1[i], select, multiplicative_selection);
    }

    for (size_t i = 0; i < pop_2.size(); ++i) {
      fitness_pop_2[i] = calc_fitness(pop_2[i], select, multiplicative_selection);
    }
  }
...
List simulate_migration_cpp(NumericVector input_population_1,
  ...
  if (input_population_1[0] > -1e4) {
    ...
    Pop_1 = convert_NumericVector_to_fishVector<FOCAL_ANIMAL, junction>(input_population_1);
    Pop_2 = convert_NumericVector_to_fishVector<FOCAL_ANIMAL, junction>(input_population_2);
